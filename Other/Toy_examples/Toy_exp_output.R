...
lamb <- seq(0.1, 6, length = 1e3)
delta_is <- seq(0.85, 18, length = length(lamb))
delta_px <- seq(1, 0.5, length = length(lamb))
...
pdf(file = "ess_plot_exp.pdf", height = 8, width = 10)
par(mfrow = c(2,2))
...
plot(lamb, rel_eff, type = 'l', xlab = "lambda", ylab = "rel_ess", 
...
#  MYMALA samples 
mala.is <- mymala(curr_val, iter, lambda.val <- 1e-4, delta_is)
mala.px <- pxmala(curr_val, iter, lambda.val <- 1e-4, delta_px <- 1.25)
