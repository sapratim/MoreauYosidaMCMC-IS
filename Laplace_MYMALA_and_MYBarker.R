...
# Density plots
pdf("plot_del5_MY.pdf", height = 6, width = 12)
par(mfrow = c(1,2))
...
mala.del1 <- mymala(in_val, iter, lambda.vec[2], delta.vec[1])
mala.del2 <- mymala(in_val, iter, lambda.vec[2], delta.vec[2])
mala.del3 <- mymala(in_val, iter, lambda.vec[2], delta.vec[3])
mala.del <- list(mala.del1, mala.del2, mala.del3)

barker.del1 <- mybarker(in_val, iter, lambda.vec[2], delta.vec[1])
barker.del2 <- mybarker(in_val, iter, lambda.vec[2], delta.vec[2])
barker.del3 <- mybarker(in_val, iter, lambda.vec[2], delta.vec[3])
barker.del <- list(barker.del1, barker.del2, barker.del3)

pdf("plot_lam_MY.pdf", height = 6, width = 15)
